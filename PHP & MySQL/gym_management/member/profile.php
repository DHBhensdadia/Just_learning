...
// Fetch current member data (GET request or failed POST)
...
if ($stmt = mysqli_prepare($link, $sql)) {
    ...
    if (mysqli_stmt_execute($stmt)) {
        ...
        if (mysqli_num_rows($result) == 1) {
            ...
            // Only overwrite if not a failed POST (preserve $inputs)
            if (count($errors) === 0) {
                $name = $row["name"];
                $phone = $row["phone"];
            }
...
require BASE_PATH . '/frontend/views/member/profile.php';
